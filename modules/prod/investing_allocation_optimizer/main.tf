...
data "template_file" "global_config_investing_allocation_optimizer_prod" {
  template = file(var.config_template_path)
  vars     = local.enriched_template_vars
}

resource "local_file" "global_config_file_investing_allocation_optimizer_prod" {
  content    = data.template_file.global_config_investing_allocation_optimizer_prod.rendered
